...
function getProduct($n, $sex)
{
    global $conn;
    require('db.php');
    $query = "SELECT * FROM sanpham WHERE gioitinh like '%$sex%' AND (loai = 'Quần' OR loai = 'Áo') or loai='Phụ Kiện' ORDER BY masanpham desc limit $n";
...
function getRelatedProduct($type, $n)
{
    global $conn;
    require('db.php');
    $query = "SELECT * FROM sanpham WHERE loai='$type' ORDER BY masanpham limit $n";
    $result = pg_query($conn, $query);
    while ($row = pg_fetch_assoc($result)) {
...
{
    global $conn;
    $sql = "SELECT * from sanpham order by masanpham asc limit 5";
...
function getProductAll($type)
{
    ...
    $query = "SELECT * FROM sanpham WHERE (gioitinh like '%$type%') AND (loai = 'Quần' OR loai = 'Áo' OR loai = 'Phụ Kiện') ORDER BY masanpham";
    $result = pg_query($conn, $query);
